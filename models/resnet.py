...
from PCANorm import myBatchNorm, myPCANorm
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, Norm, in_planes, planes, stride=1):
        ...
        self.bn1 = Norm(planes, affine=True) if Norm in [nn.BatchNorm2d, nn.InstanceNorm2d, myBatchNorm, myPCANorm] else Norm(32, planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = Norm(planes, affine=True) if Norm in [nn.BatchNorm2d, nn.InstanceNorm2d, myBatchNorm, myPCANorm] else Norm(32, planes)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False),
                Norm(self.expansion*planes, affine=True) if Norm in [nn.BatchNorm2d, nn.InstanceNorm2d, myBatchNorm, myPCANorm] else Norm(32, self.expansion*planes)
            )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, Norm, in_planes, planes, stride=1):
        ...
        self.bn3 = Norm(self.expansion*planes, affine=True) if Norm in [nn.BatchNorm2d, nn.InstanceNorm2d, myBatchNorm, myPCANorm] else Norm(32, self.expansion*planes)
...
class ResNet(nn.Module):
    def __init__(self, Norm, block, num_blocks, num_classes=10):
        ...
        self.bn1 = Norm(64, affine=True) if Norm in [nn.BatchNorm2d, nn.InstanceNorm2d, myBatchNorm, myPCANorm] else Norm(32, 64)
